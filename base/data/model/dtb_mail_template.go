...
func (o *DTBMailTemplate) Creator(mods ...qm.QueryMod) dtbMemberQuery {
...
func (dtbMailTemplateL) LoadCreator(ctx context.Context, e boil.ContextExecutor, singular bool, maybeDTBMailTemplate interface{}, mods queries.Applicator) error {
	var slice []*DTBMailTemplate
	var object *DTBMailTemplate

	if singular {
		object = maybeDTBMailTemplate.(*DTBMailTemplate)
	} else {
		slice = *maybeDTBMailTemplate.(*[]*DTBMailTemplate)
	}

	args := make([]interface{}, 0, 1)
	if singular {
		if object.R == nil {
			object.R = &dtbMailTemplateR{}
		}
	...
	if singular {
		...
		foreign.R.CreatorDTBMailTemplates = append(foreign.R.CreatorDTBMailTemplates, object)
		return nil
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if queries.Equal(local.CreatorID, foreign.ID) {
				...
				foreign.R.CreatorDTBMailTemplates = append(foreign.R.CreatorDTBMailTemplates, local)
				break
			}
		}
	}
...
// SetCreator of the dtbMailTemplate to the related item.
// Sets o.R.Creator to related.
// Adds o to related.R.CreatorDTBMailTemplates.
func (o *DTBMailTemplate) SetCreator(ctx context.Context, exec boil.ContextExecutor, insert bool, related *DTBMember) error {
	var err error
	if insert {
	...
	if related.R == nil {
		related.R = &dtbMemberR{
			CreatorDTBMailTemplates: DTBMailTemplateSlice{o},
		}
	} else {
		related.R.CreatorDTBMailTemplates = append(related.R.CreatorDTBMailTemplates, o)
	}
...
func (o *DTBMailTemplate) RemoveCreator(ctx context.Context, exec boil.ContextExecutor, related *DTBMember) error {
	var err error
	...
	for i, ri := range related.R.CreatorDTBMailTemplates {
		if queries.Equal(o.CreatorID, ri.CreatorID) {
			continue
		}

		ln := len(related.R.CreatorDTBMailTemplates)
		if ln > 1 && i < ln-1 {
			related.R.CreatorDTBMailTemplates[i] = related.R.CreatorDTBMailTemplates[ln-1]
		}
		related.R.CreatorDTBMailTemplates = related.R.CreatorDTBMailTemplates[:ln-1]
		break
	}
	return nil
}
