...
func testDTBDeliveryToOneDTBMemberUsingCreator(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadCreator(ctx, tx, false, (*[]*DTBDelivery)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testDTBDeliveryToOneSetOpDTBMemberUsingCreator(t *testing.T) {
	var err error
	...
	for i, x := range []*DTBMember{&b, &c} {
		...
		if x.R.CreatorDTBDeliveries[0] != &a {
...
func testDTBDeliveryToOneRemoveOpDTBMemberUsingCreator(t *testing.T) {
	var err error
	...
	if len(b.R.CreatorDTBDeliveries) != 0 {
