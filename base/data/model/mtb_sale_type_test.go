...
func testMTBSaleTypeToManySaleTypeDTBProductClasses(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.SaleTypeDTBProductClasses().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := MTBSaleTypeSlice{&a}
	if err = a.L.LoadSaleTypeDTBProductClasses(ctx, tx, false, (*[]*MTBSaleType)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.SaleTypeDTBProductClasses); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.SaleTypeDTBProductClasses = nil
	if err = a.L.LoadSaleTypeDTBProductClasses(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.SaleTypeDTBProductClasses); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testMTBSaleTypeToManyAddOpSaleTypeDTBProductClasses(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddSaleTypeDTBProductClasses(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.SaleTypeDTBProductClasses[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.SaleTypeDTBProductClasses[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.SaleTypeDTBProductClasses().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testMTBSaleTypeToManySetOpSaleTypeDTBProductClasses(t *testing.T) {
	var err error
	...
	err = a.SetSaleTypeDTBProductClasses(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetSaleTypeDTBProductClasses(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if a.R.SaleTypeDTBProductClasses[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.SaleTypeDTBProductClasses[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testMTBSaleTypeToManyRemoveOpSaleTypeDTBProductClasses(t *testing.T) {
	var err error
	...
	err = a.AddSaleTypeDTBProductClasses(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveSaleTypeDTBProductClasses(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.SaleTypeDTBProductClasses) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.SaleTypeDTBProductClasses[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.SaleTypeDTBProductClasses[0] != &e {
		t.Error("relationship to e should have been preserved")
	}
}
