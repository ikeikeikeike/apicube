...
func testDTBMailTemplateToOneDTBMemberUsingCreator(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()

	var local DTBMailTemplate
	var foreign DTBMember

	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &local, dtbMailTemplateDBTypes, true, dtbMailTemplateColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize DTBMailTemplate struct: %s", err)
	}
	...
	slice := DTBMailTemplateSlice{&local}
	if err = local.L.LoadCreator(ctx, tx, false, (*[]*DTBMailTemplate)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testDTBMailTemplateToOneSetOpDTBMemberUsingCreator(t *testing.T) {
	var err error
	...
	var a DTBMailTemplate
	var b, c DTBMember

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, dtbMailTemplateDBTypes, false, strmangle.SetComplement(dtbMailTemplatePrimaryKeyColumns, dtbMailTemplateColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	...
	for i, x := range []*DTBMember{&b, &c} {
		...
		if x.R.CreatorDTBMailTemplates[0] != &a {
...
func testDTBMailTemplateToOneRemoveOpDTBMemberUsingCreator(t *testing.T) {
	var err error
	...
	if len(b.R.CreatorDTBMailTemplates) != 0 {
