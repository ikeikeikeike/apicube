...
func (o *DTBProductTag) Creator(mods ...qm.QueryMod) dtbMemberQuery {
	queryMods := []qm.QueryMod{
...
// loaded structs of the objects. This is for an N-1 relationship.
func (dtbProductTagL) LoadCreator(ctx context.Context, e boil.ContextExecutor, singular bool, maybeDTBProductTag interface{}, mods queries.Applicator) error {
	var slice []*DTBProductTag
	var object *DTBProductTag
	...
	if singular {
		...
		foreign.R.CreatorDTBProductTags = append(foreign.R.CreatorDTBProductTags, object)
		return nil
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if queries.Equal(local.CreatorID, foreign.ID) {
				...
				foreign.R.CreatorDTBProductTags = append(foreign.R.CreatorDTBProductTags, local)
				break
			}
		}
	}
...
// SetCreator of the dtbProductTag to the related item.
// Sets o.R.Creator to related.
// Adds o to related.R.CreatorDTBProductTags.
func (o *DTBProductTag) SetCreator(ctx context.Context, exec boil.ContextExecutor, insert bool, related *DTBMember) error {
	var err error
	if insert {
		if err = related.Insert(ctx, exec, boil.Infer()); err != nil {
			return errors.Wrap(err, "failed to insert into foreign table")
	...
	if related.R == nil {
		related.R = &dtbMemberR{
			CreatorDTBProductTags: DTBProductTagSlice{o},
		}
	} else {
		related.R.CreatorDTBProductTags = append(related.R.CreatorDTBProductTags, o)
	}
...
// Removes o from all passed in related items' relationships struct (Optional).
func (o *DTBProductTag) RemoveCreator(ctx context.Context, exec boil.ContextExecutor, related *DTBMember) error {
	var err error
	...
	for i, ri := range related.R.CreatorDTBProductTags {
		if queries.Equal(o.CreatorID, ri.CreatorID) {
			continue
		}

		ln := len(related.R.CreatorDTBProductTags)
		if ln > 1 && i < ln-1 {
			related.R.CreatorDTBProductTags[i] = related.R.CreatorDTBProductTags[ln-1]
		}
		related.R.CreatorDTBProductTags = related.R.CreatorDTBProductTags[:ln-1]
		break
	}
	return nil
}
