...
func (o *DTBProductStock) ProductClass(mods ...qm.QueryMod) dtbProductClassQuery {
...
func (dtbProductStockL) LoadProductClass(ctx context.Context, e boil.ContextExecutor, singular bool, maybeDTBProductStock interface{}, mods queries.Applicator) error {
	...
	if singular {
		...
		foreign.R.ProductClassDTBProductStocks = append(foreign.R.ProductClassDTBProductStocks, object)
		return nil
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if queries.Equal(local.ProductClassID, foreign.ID) {
				...
				foreign.R.ProductClassDTBProductStocks = append(foreign.R.ProductClassDTBProductStocks, local)
				break
			}
		}
	}
...
func (o *DTBProductStock) SetProductClass(ctx context.Context, exec boil.ContextExecutor, insert bool, related *DTBProductClass) error {
	var err error
	if insert {
	...
	if related.R == nil {
		related.R = &dtbProductClassR{
			ProductClassDTBProductStocks: DTBProductStockSlice{o},
		}
	} else {
		related.R.ProductClassDTBProductStocks = append(related.R.ProductClassDTBProductStocks, o)
	}
...
func (o *DTBProductStock) RemoveProductClass(ctx context.Context, exec boil.ContextExecutor, related *DTBProductClass) error {
	var err error
	...
	for i, ri := range related.R.ProductClassDTBProductStocks {
		if queries.Equal(o.ProductClassID, ri.ProductClassID) {
			continue
		}

		ln := len(related.R.ProductClassDTBProductStocks)
		if ln > 1 && i < ln-1 {
