...
func (o *DTBCategory) Creator(mods ...qm.QueryMod) dtbMemberQuery {
...
func (dtbCategoryL) LoadCreator(ctx context.Context, e boil.ContextExecutor, singular bool, maybeDTBCategory interface{}, mods queries.Applicator) error {
	...
	if singular {
		...
		foreign.R.CreatorDTBCategories = append(foreign.R.CreatorDTBCategories, object)
		return nil
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if queries.Equal(local.CreatorID, foreign.ID) {
				...
				foreign.R.CreatorDTBCategories = append(foreign.R.CreatorDTBCategories, local)
				break
			}
		}
	}
...
// SetCreator of the dtbCategory to the related item.
// Sets o.R.Creator to related.
// Adds o to related.R.CreatorDTBCategories.
func (o *DTBCategory) SetCreator(ctx context.Context, exec boil.ContextExecutor, insert bool, related *DTBMember) error {
	var err error
	if insert {
	...
	if related.R == nil {
		related.R = &dtbMemberR{
			CreatorDTBCategories: DTBCategorySlice{o},
		}
	} else {
		related.R.CreatorDTBCategories = append(related.R.CreatorDTBCategories, o)
	}
...
func (o *DTBCategory) RemoveCreator(ctx context.Context, exec boil.ContextExecutor, related *DTBMember) error {
	var err error
	...
	for i, ri := range related.R.CreatorDTBCategories {
		if queries.Equal(o.CreatorID, ri.CreatorID) {
			continue
		}

		ln := len(related.R.CreatorDTBCategories)
		if ln > 1 && i < ln-1 {
			related.R.CreatorDTBCategories[i] = related.R.CreatorDTBCategories[ln-1]
		}
		related.R.CreatorDTBCategories = related.R.CreatorDTBCategories[:ln-1]
		break
	}
	return nil
}
