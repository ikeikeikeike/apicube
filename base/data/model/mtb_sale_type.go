...
// SaleTypeDTBProductClasses retrieves all the dtb_product_class's DTBProductClasses with an executor via sale_type_id column.
func (o *MTBSaleType) SaleTypeDTBProductClasses(mods ...qm.QueryMod) dtbProductClassQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("`dtb_product_class`.`sale_type_id`=?", o.ID),
	)
...
// LoadSaleTypeDTBProductClasses allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (mtbSaleTypeL) LoadSaleTypeDTBProductClasses(ctx context.Context, e boil.ContextExecutor, singular bool, maybeMTBSaleType interface{}, mods queries.Applicator) error {
	var slice []*MTBSaleType
	var object *MTBSaleType
	...
	query := NewQuery(qm.From(`dtb_product_class`), qm.WhereIn(`sale_type_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.SaleTypeDTBProductClasses = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.ID, foreign.SaleTypeID) {
				local.R.SaleTypeDTBProductClasses = append(local.R.SaleTypeDTBProductClasses, foreign)
				if foreign.R == nil {
					foreign.R = &dtbProductClassR{}
				}
				foreign.R.SaleType = local
...
// AddSaleTypeDTBProductClasses adds the given related objects to the existing relationships
// of the mtb_sale_type, optionally inserting them as new records.
// Appends related to o.R.SaleTypeDTBProductClasses.
// Sets related.R.SaleType appropriately.
func (o *MTBSaleType) AddSaleTypeDTBProductClasses(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*DTBProductClass) error {
	var err error
	for _, rel := range related {
		if insert {
			queries.Assign(&rel.SaleTypeID, o.ID)
	...
	if o.R == nil {
		o.R = &mtbSaleTypeR{
			SaleTypeDTBProductClasses: related,
		}
	} else {
		o.R.SaleTypeDTBProductClasses = append(o.R.SaleTypeDTBProductClasses, related...)
	}
...
// SetSaleTypeDTBProductClasses removes all previously related items of the
// mtb_sale_type replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.SaleType's SaleTypeDTBProductClasses accordingly.
// Replaces o.R.SaleTypeDTBProductClasses with related.
// Sets related.R.SaleType's SaleTypeDTBProductClasses accordingly.
func (o *MTBSaleType) SetSaleTypeDTBProductClasses(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*DTBProductClass) error {
	query := "update `dtb_product_class` set `sale_type_id` = null where `sale_type_id` = ?"
	values := []interface{}{o.ID}
	if boil.DebugMode {
		fmt.Fprintln(boil.DebugWriter, query)
		fmt.Fprintln(boil.DebugWriter, values)
	...
	if o.R != nil {
		for _, rel := range o.R.SaleTypeDTBProductClasses {
			queries.SetScanner(&rel.SaleTypeID, nil)
			if rel.R == nil {
				continue
			}
		...
		o.R.SaleTypeDTBProductClasses = nil
	}
	return o.AddSaleTypeDTBProductClasses(ctx, exec, insert, related...)
}

// RemoveSaleTypeDTBProductClasses relationships from objects passed in.
// Removes related items from R.SaleTypeDTBProductClasses (uses pointer comparison, removal does not keep order)
// Sets related.R.SaleType.
func (o *MTBSaleType) RemoveSaleTypeDTBProductClasses(ctx context.Context, exec boil.ContextExecutor, related ...*DTBProductClass) error {
	var err error
	for _, rel := range related {
		queries.SetScanner(&rel.SaleTypeID, nil)
		if rel.R != nil {
	...
	for _, rel := range related {
		for i, ri := range o.R.SaleTypeDTBProductClasses {
			if rel != ri {
				continue
			}

			ln := len(o.R.SaleTypeDTBProductClasses)
			if ln > 1 && i < ln-1 {
				o.R.SaleTypeDTBProductClasses[i] = o.R.SaleTypeDTBProductClasses[ln-1]
			}
			o.R.SaleTypeDTBProductClasses = o.R.SaleTypeDTBProductClasses[:ln-1]
			break
		}
	}
