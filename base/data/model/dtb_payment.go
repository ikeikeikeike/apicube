...
func (o *DTBPayment) Creator(mods ...qm.QueryMod) dtbMemberQuery {
...
func (dtbPaymentL) LoadCreator(ctx context.Context, e boil.ContextExecutor, singular bool, maybeDTBPayment interface{}, mods queries.Applicator) error {
	...
	if singular {
		...
		foreign.R.CreatorDTBPayments = append(foreign.R.CreatorDTBPayments, object)
		return nil
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if queries.Equal(local.CreatorID, foreign.ID) {
				...
				foreign.R.CreatorDTBPayments = append(foreign.R.CreatorDTBPayments, local)
				break
			}
		}
	}
...
// SetCreator of the dtbPayment to the related item.
// Sets o.R.Creator to related.
// Adds o to related.R.CreatorDTBPayments.
func (o *DTBPayment) SetCreator(ctx context.Context, exec boil.ContextExecutor, insert bool, related *DTBMember) error {
	var err error
	if insert {
	...
	if related.R == nil {
		related.R = &dtbMemberR{
			CreatorDTBPayments: DTBPaymentSlice{o},
		}
	} else {
		related.R.CreatorDTBPayments = append(related.R.CreatorDTBPayments, o)
	}
...
func (o *DTBPayment) RemoveCreator(ctx context.Context, exec boil.ContextExecutor, related *DTBMember) error {
	var err error
	...
	for i, ri := range related.R.CreatorDTBPayments {
		if queries.Equal(o.CreatorID, ri.CreatorID) {
			continue
		}

		ln := len(related.R.CreatorDTBPayments)
		if ln > 1 && i < ln-1 {
			related.R.CreatorDTBPayments[i] = related.R.CreatorDTBPayments[ln-1]
		}
		related.R.CreatorDTBPayments = related.R.CreatorDTBPayments[:ln-1]
		break
	}
	return nil
}
