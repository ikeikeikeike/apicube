...
var DTBProductRels = struct {
	...
	ProductDTBProductClasses           string
	...
	ProductDTBProductClasses:           "ProductDTBProductClasses",
...
type dtbProductR struct {
	...
	ProductDTBProductClasses           DTBProductClassSlice
...
func (o *DTBProduct) Creator(mods ...qm.QueryMod) dtbMemberQuery {
	queryMods := []qm.QueryMod{
...
// ProductDTBProductClasses retrieves all the dtb_product_class's DTBProductClasses with an executor via product_id column.
func (o *DTBProduct) ProductDTBProductClasses(mods ...qm.QueryMod) dtbProductClassQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("`dtb_product_class`.`product_id`=?", o.ID),
	)
...
// loaded structs of the objects. This is for an N-1 relationship.
func (dtbProductL) LoadCreator(ctx context.Context, e boil.ContextExecutor, singular bool, maybeDTBProduct interface{}, mods queries.Applicator) error {
	var slice []*DTBProduct
	var object *DTBProduct
	...
	if singular {
		...
		foreign.R.CreatorDTBProducts = append(foreign.R.CreatorDTBProducts, object)
		return nil
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if queries.Equal(local.CreatorID, foreign.ID) {
				...
				foreign.R.CreatorDTBProducts = append(foreign.R.CreatorDTBProducts, local)
				break
			}
		}
	}
...
// LoadProductDTBProductClasses allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (dtbProductL) LoadProductDTBProductClasses(ctx context.Context, e boil.ContextExecutor, singular bool, maybeDTBProduct interface{}, mods queries.Applicator) error {
	var slice []*DTBProduct
	var object *DTBProduct
	...
	query := NewQuery(qm.From(`dtb_product_class`), qm.WhereIn(`product_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.ProductDTBProductClasses = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.ID, foreign.ProductID) {
				local.R.ProductDTBProductClasses = append(local.R.ProductDTBProductClasses, foreign)
				if foreign.R == nil {
					foreign.R = &dtbProductClassR{}
				}
				foreign.R.Product = local
...
// SetCreator of the dtbProduct to the related item.
// Sets o.R.Creator to related.
// Adds o to related.R.CreatorDTBProducts.
func (o *DTBProduct) SetCreator(ctx context.Context, exec boil.ContextExecutor, insert bool, related *DTBMember) error {
	var err error
	if insert {
		if err = related.Insert(ctx, exec, boil.Infer()); err != nil {
			return errors.Wrap(err, "failed to insert into foreign table")
	...
	if related.R == nil {
		related.R = &dtbMemberR{
			CreatorDTBProducts: DTBProductSlice{o},
		}
	} else {
		related.R.CreatorDTBProducts = append(related.R.CreatorDTBProducts, o)
	}
...
// Removes o from all passed in related items' relationships struct (Optional).
func (o *DTBProduct) RemoveCreator(ctx context.Context, exec boil.ContextExecutor, related *DTBMember) error {
	var err error
	...
	for i, ri := range related.R.CreatorDTBProducts {
		if queries.Equal(o.CreatorID, ri.CreatorID) {
			continue
		}

		ln := len(related.R.CreatorDTBProducts)
		if ln > 1 && i < ln-1 {
			related.R.CreatorDTBProducts[i] = related.R.CreatorDTBProducts[ln-1]
		}
		related.R.CreatorDTBProducts = related.R.CreatorDTBProducts[:ln-1]
		break
	}
	return nil
}
...
// AddProductDTBProductClasses adds the given related objects to the existing relationships
// of the dtb_product, optionally inserting them as new records.
// Appends related to o.R.ProductDTBProductClasses.
// Sets related.R.Product appropriately.
func (o *DTBProduct) AddProductDTBProductClasses(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*DTBProductClass) error {
	var err error
	for _, rel := range related {
		if insert {
			queries.Assign(&rel.ProductID, o.ID)
	...
	if o.R == nil {
		o.R = &dtbProductR{
			ProductDTBProductClasses: related,
		}
	} else {
		o.R.ProductDTBProductClasses = append(o.R.ProductDTBProductClasses, related...)
	}
...
// SetProductDTBProductClasses removes all previously related items of the
// dtb_product replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Product's ProductDTBProductClasses accordingly.
// Replaces o.R.ProductDTBProductClasses with related.
// Sets related.R.Product's ProductDTBProductClasses accordingly.
func (o *DTBProduct) SetProductDTBProductClasses(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*DTBProductClass) error {
	query := "update `dtb_product_class` set `product_id` = null where `product_id` = ?"
	values := []interface{}{o.ID}
	if boil.DebugMode {
		fmt.Fprintln(boil.DebugWriter, query)
		fmt.Fprintln(boil.DebugWriter, values)
	...
	if o.R != nil {
		for _, rel := range o.R.ProductDTBProductClasses {
			queries.SetScanner(&rel.ProductID, nil)
			if rel.R == nil {
				continue
			}
		...
		o.R.ProductDTBProductClasses = nil
	}
	return o.AddProductDTBProductClasses(ctx, exec, insert, related...)
}

// RemoveProductDTBProductClasses relationships from objects passed in.
// Removes related items from R.ProductDTBProductClasses (uses pointer comparison, removal does not keep order)
// Sets related.R.Product.
func (o *DTBProduct) RemoveProductDTBProductClasses(ctx context.Context, exec boil.ContextExecutor, related ...*DTBProductClass) error {
	var err error
	for _, rel := range related {
		queries.SetScanner(&rel.ProductID, nil)
		if rel.R != nil {
	...
	for _, rel := range related {
		for i, ri := range o.R.ProductDTBProductClasses {
			if rel != ri {
				continue
			}

			ln := len(o.R.ProductDTBProductClasses)
			if ln > 1 && i < ln-1 {
				o.R.ProductDTBProductClasses[i] = o.R.ProductDTBProductClasses[ln-1]
			}
			o.R.ProductDTBProductClasses = o.R.ProductDTBProductClasses[:ln-1]
			break
		}
	}
