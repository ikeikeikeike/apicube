...
func testDTBAuthorityRoleToOneDTBMemberUsingCreator(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadCreator(ctx, tx, false, (*[]*DTBAuthorityRole)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testDTBAuthorityRoleToOneSetOpDTBMemberUsingCreator(t *testing.T) {
	var err error
	...
	for i, x := range []*DTBMember{&b, &c} {
		...
		if x.R.CreatorDTBAuthorityRoles[0] != &a {
...
func testDTBAuthorityRoleToOneRemoveOpDTBMemberUsingCreator(t *testing.T) {
	var err error
	...
	if len(b.R.CreatorDTBAuthorityRoles) != 0 {
