...
func testDTBProductImageToOneDTBMemberUsingCreator(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := DTBProductImageSlice{&local}
	if err = local.L.LoadCreator(ctx, tx, false, (*[]*DTBProductImage)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testDTBProductImageToOneSetOpDTBMemberUsingCreator(t *testing.T) {
	var err error
	...
	for i, x := range []*DTBMember{&b, &c} {
		...
		if x.R.CreatorDTBProductImages[0] != &a {
			t.Error("failed to append to foreign relationship struct")
		}
...
func testDTBProductImageToOneRemoveOpDTBMemberUsingCreator(t *testing.T) {
	var err error
	...
	if len(b.R.CreatorDTBProductImages) != 0 {
		t.Error("failed to remove a from b's relationships")
	}
}
